...
type Periodo = 'hoje' | 'ontem' | '7dias' | '30dias' | 'mes' | 'mes_passado';
...
function Cartao({
  ...
  return (
    ...
    >
      ...
      {rodape && (
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5">{rodape}</p>
      )}
    </div>
  );
}
 ...
 */
function Barras({ dados }: { dados: Array<{ dia: string; investido: number; resultados: number }> }) {
        ...
        {dados.map((d) => (
          <div
            ...
            title={`${new Date(d.dia + 'T12:00:00').toLocaleDateString('pt-BR', {
              day: '2-digit',
              month: '2-digit',
            })} · ${formatarDinheiro(d.investido)}${d.resultados ? ` · ${d.resultados} result.` : ''}`}
